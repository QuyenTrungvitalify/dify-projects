...
const EN: Dict = {
  ...
  intakeHint: 'Upload or paste a workflow YAML that isn’t in a project yet, then choose what to do with it. It is validated first.',
  intakeActionLabel: 'What to do',
  intakeActionBase: 'Use as base',
  intakeActionDistill: 'Distill to shelf',
  intakeDistillHint: 'Distill this into a reusable templates/patterns/ pattern. A review opens that you Approve — nothing lands on the shelf until then.',
  ...
  graduateBtn: 'Start build from this chat',
  ...
  promoteReviewSummary: 'Approve is the only action that writes to templates/patterns/ — review the pattern first.',
  promoteTargetLine: 'Target: {target}',
  promoteProbeLine: 'Import probe: {probe}',
  promoteRulesLine: 'Recorded {n} linter-rule candidate(s).',
...
const JA: Dict = {
  ...
  intakeHint: 'まだプロジェクトにない外部ワークフローのYAMLをアップロードまたは貼り付けて、用途を選びます。先に検証されます。',
  intakeActionLabel: '用途',
  intakeActionBase: 'ベースとして使う',
  intakeActionDistill: 'パターン棚に蒸留',
  intakeDistillHint: '再利用可能な templates/patterns/ パターンに蒸留します。承認レビューが開き、承認するまで棚には保存されません。',
  ...
  promoteReviewSummary: '「承認」だけが templates/patterns/ に書き込みます — 先にパターンを確認してください。',
  promoteTargetLine: '出力先: {target}',
  promoteProbeLine: 'インポートプローブ: {probe}',
  promoteRulesLine: 'リンター規則候補を {n} 件記録しました。',

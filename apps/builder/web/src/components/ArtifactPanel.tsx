/* ============================================================
   ArtifactPanel.tsx — right slide-in: Spec / YAML+lint / Diff / Report
   Ported from artifact.jsx. SPEC tab is editable in place (AC #3);
   the split diff renders add/del columns (AC #4). Secrets are never
   rendered — only the .secret-note reminder.
   ============================================================ */
import { useState } from 'preact/hooks';
import type { VNode } from 'preact';
import { I } from './Icon';
import { SPEC_MD, YAML_LINES, LINTERS, LINTERS_FAIL, DIFF, REPORT } from '../data';
import type { ArtifactTab, Scenario } from '../types';

function SpecTab() {
  const [val, setVal] = useState(SPEC_MD);
    ...
    <div>
      ...
      <div className="spec-bar">
        <button className="btn primary" disabled={saved}
          onClick={() => setSaved(true)} style={saved ? { opacity: 0.5 } : undefined}
        >
          <I.save />Save spec
        </button>
...
function YamlTab({ scenario }: { scenario: Scenario }) {
...
function ReportTab({ deploy }: { deploy: string }) {
...
export function ArtifactPanel({ tab, setTab, scenario, deploy, available, onClose }: {
  tab: ArtifactTab;
    ...
    <aside className="artifact">
      <div className="artifact-head">
        ...
        <button className="icon-btn artifact-close" style={{ marginLeft: 'auto' }}
          onClick={onClose} title="Hide panel"
        ><I.close /></button>
      </div>

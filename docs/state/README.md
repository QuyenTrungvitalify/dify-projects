...
| [templates-and-promotion.md](templates-and-promotion.md) | Kệ mẫu có tầng nào, cái gì tìm ra chúng, một thứ **lên kệ** bằng đường nào | `templates/{patterns,library,_base,probes}/` · `promote.ts` · `promote_gate.py` · `provenance.py` · `check_provenance.py` · `build_index.py` · `sources.py` · `sources_admin.py` · `find.py` · `corpus/sources.yml` · `INDEX.md` · `tools/dify_base/index.json` · `THIRD_PARTY.md` |
...
| [ui-surface.md](ui-surface.md) | User thấy gì, làm được gì ở mỗi trạng thái; SSE truyền gì, store giữ gì, cái gì chỉ sống ở client | `web/src/store.ts` · `web/src/sse-client.ts` · `web/src/lib/` **trừ `shelf.ts` + `sidebar-prefs.ts`**: `i18n.ts` · `gate-foot.ts` · `thread-persist.ts` · `crumb.ts` · `phase.ts` · `markdown.ts` · `diff-parser.ts` · `slug.ts` · `attachments.ts` · `dev.ts` · `promote-visibility.ts` · `notify.ts` |
...
- `routes/tasks.ts`: sở hữu **vòng đời build** — `/api/tasks`, `/confirm`, `/reply`, `/cancel`,
  `/restore`, `/live-test`, `PATCH /:id`, `POST /:id/undo-promote`, `DELETE /api/tasks/:id`,
  `dispatch`/`failSafe`, và rẽ nhánh `task.kind === 'promote'`.
...
| Đăng nhập Claude trong app | `lib/claude-auth.ts` *(đọc: `claude auth status --json` — trả JSON trên stdout với exit **khác 0** khi chưa đăng nhập, nên tín hiệu là JSON chứ không phải exit code; ghi: chạy `claude auth login` và mang mã dán về. CLI không dùng callback localhost và **bỏ qua stdin**, nên không thể ống mã vào)* · `routes/auth.ts` *(`GET /api/auth/status` · `POST /api/auth/login` → trả URL trang đăng nhập · `.../login/code` · `.../login/cancel`; mount cho MỌI run — không phải bề mặt dev — vì đối tượng là user bản sạch không có terminal. Một phiên login tại một thời điểm, cái thứ hai nhận 409)* |
| Dev | `dev-rebuild.ts` · `routes/dev.ts` *(gồm `GET /api/dev/shelf`)* · `lib/settings.ts` *(⚙ Settings modal dưới `BUILDER_DEV` — spec 083 follow-up)* · `lib/self-update.ts` · `routes/update.ts` *(bản user-facing của dev-rebuild — update & restart trong app, mount cho MỌI run)* · **cách bật dev-mode**: file marker `.builder-dev` ở gốc repo (gitignored) khiến `scripts/update-and-run.command` export `BUILDER_DEV=1` và mở `?dev=1`; máy không có file thì chạy hoàn toàn không bề mặt dev |
| Ask & hội thoại | `ask-ledger.ts` *(bảng một dòng/ask ghi kích thước seed + token, để "tối ưu ask còn hoạt động không" trả lời được bằng cách chạy chứ không bằng một phép đo cũ)* · `ask-backfill.ts` *(dựng lại hội thoại của một build từ `chat.jsonl` trên đĩa khi localStorage của trình duyệt đã mất — LRU đuổi, xoá cache, máy khác)* · `chat-lang.ts` *(pick ở menu ⚙ rơi vào đâu: mặc định toàn cục khi chưa mở build nào, hay setting của chính build đang mở)* |
...
| Web | `web/src/api.ts` (shape HTTP + `ApiError`) · `web/src/types.ts` (`Wire*` — hợp đồng wire, không gì so nó với cái server thật phát ra) · `web/src/components/**` (gồm `App.tsx`/`Chat.tsx` — nơi mọi thứ thật sự render, và **không có test nào**) · `main.tsx` · `data.ts` · `web/src/lib/sidebar-prefs.ts` *(UI pref client: số hàng "Show N more" mỗi mục sidebar; localStorage như theme/lang, không đụng server — spec 084 follow-up)* · `web/src/lib/artifact-toc.ts` *(nửa thuần của rail mục lục trong artifact panel; nửa DOM đọc thẳng heading đã render nên không thể lệch với màn hình — `main.yml` là một khối `<pre>` nên phải sinh mục lục ở đây)* · `web/src/lib/workflow-row.ts` *(click vào một hàng workflow ở sidebar thì làm gì: chỉ mở rộng, hay mở một build)* · `web/src/lib/propose-lane.ts` *(composer có mời "xem kế hoạch trước" không — tách thuần khỏi `App.tsx` vì sai ở cả hai chiều đều IM LẶNG: mời chỗ server sẽ từ chối, hoặc giấu chỗ vốn hợp lệ)* · `web/src/lib/turn-busy.ts` *(nhận ra đúng 409 "đã có turn đang chạy" giữa các 409 khác, dựa trên một chuỗi server cam kết giữ nguyên văn)* · `web/src/lib/cancel-confirm.ts` *(× trên hàng "In progress" hiện lời xác nhận nào — không nhánh nào bỏ qua hộp thoại, trạng thái chỉ quyết định CÂU CHỮ)* · `web/src/lib/confirm-chip.ts` *(chip Confirm-mode còn đổi được gì cho build này không — chỉ đáng một slot khi còn một boundary nó chi phối ở phía trước)* |

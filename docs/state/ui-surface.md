...
| `dev.ts` → `askCostLine` | dòng meter dưới **một** câu trả lời hoặc **một** lượt chạy phase | Số của **từng lượt**, không phải từng phase: `task.cost[phase]` là last-write-wins qua các vòng `/reply`, nên vẽ nó xuống dưới mọi run item sẽ hiện số của vòng mới nhất ba lần, hai lần sai. Phase phát `phase:cost` mỗi attempt; store **cộng dồn** vào run item đang chạy (một run có thể gồm error→retry, cả hai đều bị tính tiền). Meter dưới run item sống qua **reload cùng máy** (thread nằm trong localStorage, `cost` đi kèm). Máy khác / xoá cache: **dựng lại được** — mỗi attempt ghi `runs.jsonl` (output đã stream + cost), `GET /api/tasks/:id` trả `runs`, `buildThreadFromRuns` dựng lại run item kèm meter. Đọc có **chặn kích thước** (snapshot này fetch lại mỗi lần reconnect) và `runsDropped` nói rõ bỏ mấy lượt. **Gate cũ KHÔNG dựng lại** — snapshot của chúng chưa bao giờ được lưu, bịa ra là đặt lời vào miệng build; thread đọc thành requirement → output từng phase → gate hiện tại. Cost còn đi thêm đường riêng (`turn_cost` trong `events.jsonl` → `runCosts` → DevPanel một hàng mỗi lượt). |

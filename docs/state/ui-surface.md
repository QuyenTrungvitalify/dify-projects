...
| `dev.ts` → `askCostLine` | dòng meter dưới **một** câu trả lời hoặc **một** lượt chạy phase | Số của **từng lượt**, không phải từng phase: `task.cost[phase]` là last-write-wins qua các vòng `/reply`, nên vẽ nó xuống dưới mọi run item sẽ hiện số của vòng mới nhất ba lần, hai lần sai. Phase phát `phase:cost` mỗi attempt; store **cộng dồn** vào run item đang chạy (một run có thể gồm error→retry, cả hai đều bị tính tiền). Meter chỉ sống khi client **đang xem** — output của phase vốn không nằm trên wire, reload là mất cả run item lẫn meter (ask thì khác: có transcript trên server). |

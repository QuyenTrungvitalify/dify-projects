...
|---|---|---|
| **Seed workflow** (`{{SEED_PATH}}`) — pull từ Dify · snapshot local · upload | ① ③ | [scaffold-and-layout.md](scaffold-and-layout.md) (dựng seed) · [dify-io.md](dify-io.md) (đường pull) |
...
| **File / ảnh user đính kèm** | ① ③ | [run-artifacts.md](run-artifacts.md) (lưu trữ) · [build-lifecycle.md](build-lifecycle.md) §2 (seam bơm vào prompt) |
| **Nền tĩnh** — `AGENTS.md` · `.claude/skills/dify-build/*.md` | ① ③ | *(chỉ thị cho agent, không phải trạng thái hệ thống — xem [README](README.md) §Không thuộc thư mục này)* |
...
| **Fold analyze → task** | `analysis.ts` | quyết định `analysisPattern`/`analysisFeatures`, thứ mà guard `auto`+fast ở [build-lifecycle.md](build-lifecycle.md) §2 đọc |

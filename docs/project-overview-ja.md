...
| ノード ID・変数参照の間違いに気づけない | 3 種の検証ツールで自動チェック |
...
flowchart TD
  ...
  A3 -->|"ゲート：確認"| A4["④ Test（検証・レポート）<br/>3 種の lint を通す"]
...
- ✅ **自動検証**：生成物は毎回 3 種の検証ツール（スキーマ・変数参照・プラグイン）でチェック。
...
- ✅ **コア機能 完成**：4 フェーズ＋ゲート＋検証＋ Dify 連携、すべて実装・統合済み。
- ✅ **品質強化 完了**：複数ビルド同時進行（ターン単位ロック）、UX 改善（ビルド中止・
  確認モード変更・スラッグ衝突防止）まで対応。
- ✅ **テスト済み**：ブラウザ自動 QA で主要シナリオを検証（多くが合格）。
- 📋 **残り**：自動モードの実走確認など、軽微な検証を一部残すのみ。

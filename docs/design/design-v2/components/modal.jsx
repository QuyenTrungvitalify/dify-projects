/* ============================================================
   modal.jsx — Create Project modal
   ============================================================ */
...
function CreateProjectModal({ onClose, onCreate }) {
  ...
  function removeFolder(id) { setFolders(f => f.filter(x => x.id !== id)); }
...
function slug(s) { return s.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "") || "workspace"; }

/* ============================================================
   ConfirmModal — common OK / Cancel dialog (site-styled)
   props: title, message, okLabel, cancelLabel, danger, onOk, onCancel
   ============================================================ */
function ConfirmModal({ title, message, okLabel = "OK", cancelLabel = "Cancel", danger, onOk, onCancel }) {
  React.useEffect(() => {
    function onKey(e) {
    ...
    <div className="modal-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) onCancel(); }}>
      <div className="confirm-modal" role="alertdialog" aria-modal="true" aria-label={title}>
        ...
        <div className="confirm-foot">
          ...
          <button className={"btn " + (danger ? "danger" : "primary")} autoFocus onClick={onOk}>
            {okLabel}
          </button>
        </div>
      </div>
